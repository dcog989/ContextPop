...
- **Icons and favicons.** To show an engine icon, the extension fetches a favicon. Depending on the favicon source setting, the request goes either to the engine's own site (`site`, which reads the icon the site declares and prefers a high-resolution one), to DuckDuckGo's icon service (`duckduckgo`), or not at all (`none`). For engines imported from the browser, the extension learns the engine's site from a search you run with it, and — when you click "Refresh icons" — from a one-off background search for engines whose site is still unknown. Fetched icons are cached in `storage.local` on your device and sent without cookies or a referrer, so they are not tied to the page you are viewing. Icons are optional; engines without one display a letter.
